...
class Tonyprr_Auth_Adapter_Doctrine implements Zend_Auth_Adapter_Interface
{
    ...
    /**
     * Permite agregar condiciones de validaci�n.
     *
     * @param  array
     * @return void
     */
...
//            $result = $query->execute(array(1 => $this->identity));
            ...
            if ($resultCount > 1) {
                ...
                $authResult['messages'][] = 'Hay m�s de un registro que tienen la misma identidad.';
...
//                if ($result[0]->$this->credentialField != $this->credential) {
                if ($result[0][$this->credentialField] != $this->credential) {
                    ...
                    $authResult['messages'][] = 'Su clave no es v�lida.';
                    ...
                    $authResult['messages'][] = 'Autentificaci�n exitosa.';
                }
            }
